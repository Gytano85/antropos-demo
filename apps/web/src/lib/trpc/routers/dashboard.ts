		...
		.output(
			z.object({
				...
				profitMargin: z.array(
					z.object({ date: z.string(), margin: z.number() }),
				),
			}),
		)
		.query(async ({ ctx }) => {
			...
			// Los gastos se guardan con importe positivo, asi que hay que restarlos:
			// sumarlos daba un dia con $1000 de venta y $300 de renta como $1300, y el
			// dueño leia esa grafica como dinero disponible.
			const cashFlow = allCompleted.reduce<Record<string, number>>((acc, t) => {
				const date = t.created_at
					? new Date(t.created_at).toISOString().split("T")[0]
					: "unknown";
				const signed = t.type === "expense" ? -t.amount : t.amount;
				acc[date] = (acc[date] || 0) + signed;
				return acc;
			}, {});
			...
			return {
				...
				revenueByCategory,
				expensesByCategory,
				cashFlow: Object.entries(cashFlow).map(([date, amount]) => ({
					date,
					amount,
				})),
				profitMargin,
...
function calculateProfitMarginSeries(
	...
	return Object.entries(dailyData).map(([date, { selling, expense }]) => {
		...
		return { date, margin: Number.parseFloat(margin.toFixed(2)) };
	});
}
