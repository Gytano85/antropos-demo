...
describe("dashboard.stats", () => {
	...
	it("pending transaction (9999) is excluded from all aggregations", async () => {
		...
		// 2025-01-15 = 1000+500-300 = 1200; si la pendiente colara serian 11199.
		expect(cfMap["2025-01-15"]).toBe(1200);
	});
	...
	it("cashFlow groups by date, null created_at → 'unknown'", async () => {
		...
		// Los gastos restan: es flujo de caja, no volumen movido.
		// 2025-01-15: income 1000+500 - expense 300 = 1200
		expect(cfMap["2025-01-15"]).toBe(1200);
		// 2025-01-16: income 200+50 - expense 100 = 150
		expect(cfMap["2025-01-16"]).toBe(150);
