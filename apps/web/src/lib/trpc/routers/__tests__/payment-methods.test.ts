...
describe("paymentMethods.list", () => {
	...
	it("keeps each account's methods out of the other's list", async () => {
		...
		expect(myList.map((pm) => pm.name).sort()).toEqual(["Card", "Cash"]);
		expect(otherList).toEqual([]);
	});

	it("lets two accounts use the same method name", async () => {
		const otherCaller = createCallerFactory(paymentMethodsRouter)({
			user: makeUser("other-user"),
		});
		// El nombre era unico globalmente: el primero que creaba "Efectivo" impedia
		// que ninguna otra cuenta lo usara nunca.
		const mine = await otherCaller.create({ name: "Cash" });
		expect(mine.name).toBe("Cash");
		expect(mine.user_uid).toBe("other-user");
	});

	it("refuses to rename or delete another account's method", async () => {
		const otherCaller = createCallerFactory(paymentMethodsRouter)({
			user: makeUser("intruder"),
		});
		const [target] = await caller.list();

		await expect(
			otherCaller.update({ id: target.id, name: "Secuestrado" }),
		).rejects.toThrow();

		// Borrar responde ok por contrato (es idempotente), pero la fila ajena
		// tiene que seguir intacta: eso es lo que se verifica.
		await otherCaller.delete({ id: target.id });

		const [after] = (await caller.list()).filter((pm) => pm.id === target.id);
...
describe("paymentMethods.update", () => {
	...
	it("fails for non-existent id (output validation)", async () => {
		const before = await caller.list();
		await expect(
			caller.update({ id: 999999, name: "Ghost" }),
		).rejects.toThrow();
		const after = await caller.list();
		expect(after.length).toBe(before.length);
	});
