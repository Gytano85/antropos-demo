...
const caller = createCallerFactory(fiscalRouter)({
	user: makeUser("fiscal-test-1"),
});
...
describe("fiscal.emit", () => {
	it("rejects when fiscal settings are not configured", async () => {
		await expect(caller.emit({ order_id: 1, model: 65 })).rejects.toThrow(
...
describe("fiscal.cancel", () => {
	...
	it("rejects for non-existent invoice", async () => {
		await expect(
			caller.cancel({
				id: 999,
				reason: "This is a valid reason for cancellation",
			}),
		).rejects.toThrow();
	});
});
